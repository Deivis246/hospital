...
const Login: React.FC<LoginProps> = ({ onLogin }) => {
  ...
  const handlePatientLogin = (e: React.FormEvent) => {
    e.preventDefault();
    // Validación de ejemplo: Cédula 1700000001 con fecha 1990-05-20
    if (idNumber === '1700000001' && birthDate === '1990-05-20') {
      onLogin(UserRole.PATIENT);
    } else if (idNumber === 'admin' && birthDate === 'admin') {
      // Atajo para admin durante desarrollo/demo
    ...
    <div className="min-h-screen bg-[#f0f2f5] flex flex-col items-center justify-center p-4 font-sans">
      <div className="w-full max-w-[450px] bg-white rounded-xl shadow-lg overflow-hidden border border-gray-200">
        <div className="bg-[#0056b3] py-8 px-6 text-center text-white">
          <h1 className="text-3xl font-bold tracking-tight">Sistema de Citas Hospitalarias</h1>
          <p className="mt-2 text-blue-100 text-lg">Agenda tu cita de forma rápida y segura</p>
        </div>
      ...
      <footer className="mt-8 text-gray-500 text-sm">
        &copy; {new Date().getFullYear()} Hospital - Todos los derechos reservados
      </footer>
    </div>
  );
};
